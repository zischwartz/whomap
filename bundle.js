...
},{"./codes":3,"./de":4,"./en":5,"./fr":6,"./nl":8,"./sv":9}],8:[function(require,module,exports){
var i18n = {
  "AF": "Afghanistan",
  "AL": "Albanië",
  "DZ": "Algerije",
...
},{"./textures.js":12}],12:[function(require,module,exports){
...
var $, Circle, brewer, color, convert_countries, d3, graticule, height, path, projection, svg, textures, topojson, width, zoom, zoomed,
...
window.dev = {
  convert_countries: convert_countries,
  d3: d3,
  topojson: topojson,
  textures: textures
};
...
d3.json('world-110m.json', function(error, world) {
  ...
  return d3.csv("population.csv", function(d) {
    d["pop_in_k"] = +d["pop_in_k"];
    return d;
  }, function(err, population) {
    var countries, dot_radius_scale, dot_size_scale, end, extent, j, l, legend, len, len1, mid, neighbors, pop_by_name, samples, t, ticks, x;
    ...
    svg.selectAll('.country').data(countries).enter().insert('path', '.graticule').attr('class', 'country').attr('d', path).style('fill', function(d, i) {
      var circle_texture, dot_radius, dot_size, name, pop_data;
      name = convert_countries.getName(d.id, "EN");
      d["name"] = name;
      pop_data = pop_by_name[name];
      d["pop_in_k"] = pop_data != null ? pop_data["pop_in_k"] : void 0;
      if (pop_data) {
      ...
      color(d.color = d3.max(neighbors[i], function(n) {
    ...
    svg.selectAll(".country").append("svg:title").text(function(d) {
      return d.name + ": " + (Math.round(d.pop_in_k / 1000)) + "m";
    });
    ...
    console.log(mid);
    samples = samples.slice(0, 3).concat(mid, end);
    legend = svg.selectAll('.legend').data(samples).enter().append('g').attr('class', 'legend').attr('transform', function(d, i) {
      return 'translate(36,' + (i * 25 + 600) + ')';
    });
    legend.append('rect').attr('x', 36).attr('width', 36).attr('height', 18).style('fill', function(d) {
      var circle_texture;
      circle_texture = textures.circles().radius(d.r).size(d.s).complement().fill("#d62728");
      svg.call(circle_texture);
      return circle_texture.url();
    });
    legend.append('text').attr('x', 24).attr('y', 9).attr('dy', '.25em').style('text-anchor', 'end').text(function(d) {
      return d.p + "m";
    });
...
Circle = (function() {
  function Circle(maxRadius, padding, width1, height1, autoadd) {
    this.maxRadius = maxRadius;
    this.padding = padding;
    this.width = width1;
    this.height = height1;
    this.autoadd = autoadd != null ? autoadd : true;
    this.make = bind(this.make, this);
    this.quadtree = d3.geom.quadtree().extent([[0, 0], [this.width, this.height]])([]);
    this.searchRadius = this.maxRadius * 2;
  }

  Circle.prototype.make = function(k) {
    var best, bestDistance, bestX, bestY, i, minDistance, rx1, rx2, ry1, ry2, x, y;
    if (typeof k !== 'number') {
      throw "Need a number";
      return;
    }
    bestDistance = i = 0;
    while (i < k || bestDistance < this.padding) {
      x = Math.random() * this.width;
      y = Math.random() * this.height;
      rx1 = x - this.searchRadius;
      rx2 = x + this.searchRadius;
      ry1 = y - this.searchRadius;
      ry2 = y + this.searchRadius;
      minDistance = this.maxRadius;
      this.quadtree.visit(function(quad, x1, y1, x2, y2) {
        var d, d2, dx, dy, p, r2;
        if (p = quad.point) {
          dx = x - p[0];
          dy = y - p[1];
    ...
    best = [bestX, bestY, bestDistance - this.padding];
    if (this.autoadd) {
      this.quadtree.add(best);
    }
    return best;
  };
...
},{"./brewer.js":1,"d3":2,"i18n-iso-countries":7,"jquery":10,"textures":11,"topojson":13}]},{},[14]);
